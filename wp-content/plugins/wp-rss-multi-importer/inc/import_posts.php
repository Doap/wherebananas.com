...
//  Add video if exists
	...
	$thisContent .= '
[doap_tabs style=modern-light active=1][doap_tab title="Vea el segmento"][iframe url="'.$items["mylink"].'?rel=0&autohide=0&controls=2&showinfo=0" width="605" height="315" frameborder="0" marginheight="2"][/doap_tab][doap_tab title="Headlines"]<div style="background-color:#fff;padding:8px;border-radius:5px;"><h3 style="margin-bottom:0px;padding-bottom:0px;padding-top:0px;margin-top:0px;">Últimas noticias</h3><div style="max-width:555px;overflow-y: auto;max-width:100%;overflow-y: auto;height: 200px;border:0px;margin-bottom:0px;margin-left:0px;">[doap_posts template="templates/home-news-loop.php" posts_per_page="5" tax_term="2498,2500,2490,2504,2508,2495,2506,2374,2496,43,2376,2503,2499" tax_operator="0" order="desc"]</div></div>[/doap_tab][doap_tab title="Suscribir"] <div style="background-color:#fff;color:#106d12;padding:8px;border-radius:5px;"> Ultima Noticias fue [news_updated_when] <div class="alignright" style="width:190px;height:80px;">[wordpress_social_login]</div> <h3>Suscribirse para noticias por su email</h3> <!--phplist form--></div> [/doap_tab][/doap_tabs]
[doap_spoiler title="Encuesta" style="modern-light" open="yes"][wpsqt name="Clasifica a los canales de noticias de Nicaragua" type="poll"][/doap_spoiler]
';
	

	//$thisExcerpt .= 'fix it here: /var/www/html/wp-content/plugins/wp-rss-multi-importer/inc/import_posts.php [iframe url="'.$items["mylink"].'?rel=0&autohide=0&controls=0&showinfo=0" width="205" height="150" frameborder="0" marginheight="2"]';
	}
